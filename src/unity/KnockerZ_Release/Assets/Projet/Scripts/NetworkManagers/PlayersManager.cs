...
public class PlayersManager : MonoBehaviour
{
	...
	// Caméra du serveur
	public GameObject _serverCamera;
	// Gestion des phases
	...
	void ConfigurePlayerScript (NetworkPlayer player, int cnt, string login)
	{
		...
		if (Network.player == player) {
			...
			if (cnt == 0)
			{
				...
				_playerScripts[0].gameObject.renderer.material.color = new Color(0,0,0.5f);
				_playerScripts[1].gameObject.renderer.material.color = new Color(0.5f,0,0);
			}
			else
			{
				_supportInventoryManager.ObjectTag = "PathJ2";
				_badActsInventoryManager.ObjectTag = "PathJ1";
				_playerScripts[1].gameObject.renderer.material.color = new Color(0,0,0.5f);
				_playerScripts[0].gameObject.renderer.material.color = new Color(0.5f,0,0);
			}
		...
		if(Network.isServer)
		{
			...
			// Si la partie à bien démarrée
			if(_phasesManager.startgame)
			{
				// On fonction du joueur
				if(cnt == 0)
					// On lui envoie ses statistiques
					networkView.RPC("PlayerReceiveStats", RPCMode.AllBuffered, player, _STATICS._statsJ1[0], _STATICS._statsJ1[1], _STATICS._statsJ1[2], _STATICS._statsJ1[3], _STATICS._statsJ1[4], _STATICS._statsJ1[5]);
				else
					networkView.RPC("PlayerReceiveStats", RPCMode.AllBuffered, player, _STATICS._statsJ2[0], _STATICS._statsJ2[1], _STATICS._statsJ2[2], _STATICS._statsJ2[3], _STATICS._statsJ2[4], _STATICS._statsJ2[5]);
			}
		}
	}

	// Le joueur reçoit ses statistiques
	[RPC]
	void PlayerReceiveStats(NetworkPlayer player, int pv, int rm, int rw, int gold, int pop, int xp){
		// Si on est bien le joueur qui doit etre synchronisé
		if (player == Network.player) {
			// Il récupère ses stats
			GameStats.Instance.Pv = pv;
			GameStats.Instance.RessourcesMat = rm;
			GameStats.Instance.RessourcesWeap = rw;
			GameStats.Instance.Gold = gold;
			GameStats.Instance.Population = pop;
			GameStats.Instance.Exp = xp;
		}
	}
