...
public class ResolutionsManager : MonoBehaviour
{
	...
	void Start ()
	{
		this.buttonsPositions = new Vector3[18];
	...
	public void ButtonsPosition()
	{
		...
		for (int i = 0; i < this.buttonsPositions.Length; i++)
		{
			...
			if (this.positionCoeff >= 0)
			{
				...
				this.buttonsPositions[i] = new Vector3(Screen.width * 0.35f, (Screen.height * 0.10f) * this.positionCoeff, 0f);
			...
			else
			{
				...
				this.buttonsPositions[i] = new Vector3(Screen.width * 0.35f, this.buttonsPositions[i-1].y - (Screen.height * 0.10f), 0f);
			}
		}
